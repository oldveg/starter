...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$user = $this->where('email', $identity)->users()->row();  //changed to get_user_by_identity from email

			if ($user)
			{
				$data = array(
					'identity'		=> $user->first_name.' '.$user->last_name, //{$this->config->item('identity', 'ion_auth')}
