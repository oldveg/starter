...
class Ion_auth_model extends CI_Model
{
	...
	public function forgotten_password($email)
	{
		...
		$key = $this->hash_code(microtime().$email);
		...
		$this->db->update($this->tables['users'], $update, array('email' => $email));
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$group_id = $this->valida_nivel_permissao($user->id);
     ...
     /**
     * Retorna o Nível de Permissão do Usuário passado por parâmetro
     * @author LAZARINI, Leonardo Filipe <leo.lazarini@gmail.com>
     * @param int $id
     * @return object
     */
	 function valida_nivel_permissao($id){
	 	$this->db->select($this->join['groups'].' as nivel_permissao');
	 	$this->db->where('usuarios_id', $id);
	 	$result = $this->db->get($this->tables['users_groups'])->result();
	 	return $result[0]->nivel_permissao;
	 }
	...
	**/
    ...
    /**
     * Retorna os grupos correspodentes ao usuário passado por parâmetro
     * @author LAZARINI, Leonardo Filipe <leo.lazarini@gmail.com>
     * @param int $user_id
     * @return object
     */
    function get_groups_user($user_id) {
        
        $this->db->from($this->tables['users_groups']);
